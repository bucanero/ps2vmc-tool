...
(function (root, factory) {
  ...
  function verify(psv) {
    ...
    /* signBuffer() writes into the buffer it hashes, so work on a copy and
     * leave the caller's bytes alone. */
    const computed = signBuffer(psv.slice(), SEED_OFFSET, HASH_OFFSET,
                                psv[TYPE_OFFSET])
                     .subarray(HASH_OFFSET, HASH_OFFSET + 0x14);

    return stored.every((b, i) => b === computed[i]) ? SIG_OK : SIG_BAD;
  }
